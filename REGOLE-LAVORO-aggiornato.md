...
**Quattro risposte, non tre.** *(Corretto il 28/08: INDIETRO e DIVERGENTE
stavano sulla stessa riga, e per una copia semplicemente vecchia una sessione
si è fermata a chiedere il permesso di fare l'unica cosa possibile.)*
...
| **dopo ogni passaggio** | `controlla-sintassi` + `controlla-token` + il banco della zona | come micro, più i banchi che toccano quel comportamento | come normale, più il banco che protegge l'invariante toccata |
...
`controlla-token.js`, il banco della zona toccata, e il `diff` contro l'online
prima di consegnare. *Il giro completo dei ventisei non sta in questa riga:
dal 28/08 è dello standard in su.*

**Un MICRO va dall'inizio alla consegna senza fermarsi.** Base riallineata da
soli, modifica fatta, controlli della zona, file consegnati. **Nessuna
domanda intermedia**, salvo un conflitto vero o un'ambiguità che può fare
danni. *Né audit, né analisi lunghe prima di lavorare: leggere per un'ora e
scrivere per un minuto è lo stesso spreco del giro completo per una parola.*

**Un MICRO non richiede diagnosi, audit o studio del sistema se la modifica
richiesta è già localizzata.** Se Alessandro indica una schermata, un testo,
una classe o un elemento preciso, si cerca quel punto, si modifica e si testa.
La presenza di codice critico nello stesso file **non alza il livello**: una
modifica resta MICRO finché non tocca accesso, dati, punteggio, memoria offline,
Firestore, service worker o pubblicazione.

**Non si promuove un MICRO per prudenza.** Si sale di livello solo se durante
il lavoro emerge davvero una modifica di logica o un rischio appartenente a
STANDARD o CRITICO.

E `sw.js` sale **solo se il file cambiato sta dentro `APP_SHELL`** — lì è
tecnica, non prudenza: senza nome nuovo il telefono serve la copia vecchia.

**Si sale per DOVE ATTERRA, non per prudenza.** Un ritocco di colore dentro la
schermata del punteggio è critico: conta dove atterra, non quanto è corto.
*Ma non si promuove un MICRO a critico per stare tranquilli:* la profondità dei
controlli sta al rischio vero, e un giro di banchi speso dove non serve è un
giro tolto a dove serviva.
...
*Con una sola eccezione, ed è il livello MICRO (regola 3):* lì il «pezzo» non
...
Il giro completo prima di consegnare **non si esegue nel livello MICRO**.
È obbligatorio nei livelli STANDARD e CRITICO. Un MICRO fa solo i controlli
previsti dalla regola 3, salvo che durante il lavoro emerga un motivo reale per
salire di livello.
