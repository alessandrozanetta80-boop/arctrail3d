...
# Le sorgenti (dizionario-*.py, markup.html) stanno accanto a questo file, in
# tools/. marketplace.html, index.js e dizionario.js si leggono e scrivono
# nella cartella da cui si lancia: la radice del repository.
...
for f, n in [("dizionario-a.py","a"), ("dizionario-b.py","b"), ("dizionario-c.py","c")]:
    part = load(os.path.join(QUI, f), n)
    dup = set(D) & set(part)
    if dup:
...
chieste = set()
try:
    markup = io.open(os.path.join(QUI, "markup.html"), encoding="utf-8").read()
